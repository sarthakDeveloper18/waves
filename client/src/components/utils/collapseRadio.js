        ...
        return ( 
            <div className = 'collapse_items_wrapper'>
                <List style = {{borderBottom: '1px solid #dbdbdb'}}>
                    <ListItem style = {{padding: '10px 23px 10px 0'}} onClick = {this.handleCLick}>
                        <ListItemText primary = {this.props.title} className = 'collapse_title'/>
                        {this.handleAngle()}
                    </ListItem>
                    <Collapse in = {this.state.open} timeout = 'auto' unmountOnExit>
                        <List component = 'div' disablePadding>
                            <RadioGroup onChange = {(event)=> this.handleChange(event)} aria-label = 'prices' name = 'prices' value = {this.state.value}>
                                {this.renderList()}
                            </RadioGroup>
                        </List>
                    </Collapse>
